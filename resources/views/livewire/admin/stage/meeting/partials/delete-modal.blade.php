<div x-data="{ show: false, meetingId: null, meetingTitle: '' }" x-show="show" x-cloak id="delete-meeting-modal" tabindex="-1"
    class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm" @click.away="show = false"
    @open-delete-modal.window="show = true; meetingId = $event.detail?.meetingId ?? null; meetingTitle = $event.detail?.meetingTitle ?? ''"
    ...
    <div class="relative w-full max-w-md bg-white rounded-lg shadow dark:bg-gray-800" @click.stop>
        ...
        <div class="p-4 md:p-5">
            <p class="mb-4 text-center text-sm text-gray-500 dark:text-gray-400"
                x-text="'{{ __('module_base.confirm_delete_message') }}'.replace(':name', meetingTitle)"></p>
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="button" wire:click="deleteMeeting({ meetingId: meetingId }); show = false"
