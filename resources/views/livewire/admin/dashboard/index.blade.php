<div class="min-h-screen flex flex-col bg-gray-100 lg:pl-64 dark:bg-gray-900" data-sidebar-content>
    @include('components.header')

    <main id="main-content" class="flex-1 px-4 py-6 lg:px-6">
        <div
            class="p-4 bg-white block sm:flex items-center justify-between rounded-t-lg border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
            <div class="w-full mb-1">
                <nav class="flex mb-5" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        <li class="inline-flex items-center">
                            <a href="{{ tenant_routes('admin.dashboard.index') }}"
                                class="inline-flex items-center text-gray-700 hover:text-primary-600 dark:text-gray-300 dark:hover:text-white">
                                <svg class="w-4 h-4 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M10.707 2.293a1 1 0 0 0-1.414 0l-7 7a 1 1 0 0 0 1.414 1.414L4 10.414V17a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-2a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v2a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-6.586l.293.293a1 1 0 0 0 1.414-1.414l-7-7Z" />
                                </svg>
                                {{ __('common.home') }}
                            </a>
                        </li>
                        <li>
                            <div class="flex items-center">
                                <svg class="w-6 h-6 text-gray-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                                        d="M7.293 14.707a 1 1 0 0 1 0-1.414L10.586 10 7.293 6.707a 1 1 0 0 1 1.414-1.414l4 4a 1 1 0 0 1 0 1.414l-4 4a 1 1 0 0 1-1.414 0Z"
                                        clip-rule="evenodd" />
                                </svg>
                                <span class="inline-flex items-center ml-1 text-gray-400 md:ml-2 dark:text-gray-500"
                                    aria-current="page">
                                    <svg class="w-4 h-4 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd"
                                            d="M2 11a1 1 0 011-1h2a1 1 0 011 1v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5zm6-4a1 1 0 011-1h2a1 1 0 011 1v9a1 1 0 01-1 1H9a1 1 0 01-1-1V7zm6-3a1 1 0 011-1h2a1 1 0 011 1v12a1 1 0 01-1 1h-2a1 1 0 01-1-1V4z"
                                            clip-rule="evenodd"></path>
                                    </svg>
                                    {{ __('common.dashboard') }}
                                </span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
                    {{ __('common.dashboard_title') }}</h1>
            </div>
        </div>

        <div class="mt-8 space-y-6">
            @if ($showUsersByRole && $showNotificationsPanel)
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:items-stretch">
                    <div
                        class="flex h-full min-h-0 flex-col rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div class="mb-4 shrink-0">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ __('common.dashboard_users_by_roles') }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                {{ __('common.dashboard_users_by_roles_description') }}</p>
                        </div>
                        <div id="users-by-role-chart" class="min-h-[320px] w-full shrink-0" data-chart-role="users-by-role"
                            data-labels='@json($usersByRoleLabels)' data-series='@json($usersByRoleSeries)'></div>
                    </div>
                    <div class="flex h-full min-h-0 flex-col">
                        <livewire:admin.dashboard.dashboard-notifications-panel />
                    </div>
                </div>
            @elseif ($showUsersByRole)
                <div
                    class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div class="mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ __('common.dashboard_users_by_roles') }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            {{ __('common.dashboard_users_by_roles_description') }}</p>
                    </div>
                    <div id="users-by-role-chart" class="min-h-[320px] w-full" data-chart-role="users-by-role"
                        data-labels='@json($usersByRoleLabels)' data-series='@json($usersByRoleSeries)'></div>
                </div>
            @elseif ($showNotificationsPanel)
                <livewire:admin.dashboard.dashboard-notifications-panel />
            @endif

            @if ($showPulseSection)
                <div
                    class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ __('common.dashboard_pulse_title') }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                {{ __('common.dashboard_pulse_description') }}</p>
                        </div>
                        <a href="{{ $pulseUrl }}" target="_blank" rel="noopener noreferrer"
                            class="inline-flex shrink-0 items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
                            {{ __('common.dashboard_open_pulse_full') }}
                        </a>
                    </div>
                    @if ($showPulseMetricsEmbed)
                        <div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-600">
                            <iframe src="{{ $pulseMetricsUrl }}" title="{{ __('common.dashboard_pulse_title') }}"
                                class="h-[min(75vh,900px)] w-full border-0 bg-gray-50 dark:bg-gray-900"></iframe>
                        </div>
                    @else
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            {{ __('common.dashboard_pulse_embed_unavailable') }}
                        </p>
                    @endif
                </div>
            @endif
        </div>
    </main>

    @include('components.footer')
</div>

@push('styles')
    <link href="{{ asset('css/module/dashboard.css') }}" rel="stylesheet" />
@endpush

@push('scripts-end')
    <script src="{{ asset('js/module/dashboard.js') }}"></script>
@endpush
